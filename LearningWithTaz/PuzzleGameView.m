...
@interface PuzzleGameView (Private)
- (void)orderingTile;
...
- (void)playWithImage:(UIImage *)image andSize:(NSInteger)size {
    PuzzleGame *board = [[PuzzleGame alloc] initWithSize:size];
    self.board = board;
                     ...
                     completion:^(BOOL finished){
                         if (finished) {
                             ...
                             if ([_board isBoardFinished]) {
                                 ...
                                 //take the animation function out of view controller and add it here
                                 
                                 [[NSNotificationCenter defaultCenter]postNotificationName:@"DoFinishPuzzle" object:nil userInfo:nil];
